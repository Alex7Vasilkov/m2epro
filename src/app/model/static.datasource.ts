...
export class StaticDatasource {
    private knowledges: Knowledge[] = [
        new Knowledge(1, 'Amazon Repricing Service', [
            new Article(1, 1,'Double Price updating (via the Service and M2E Pro Connector)', 'After the Product is added to Repricing Service, M2E Pro stops affecting its Price. It is made to prevent the double synchronizing of the Product Price.\n' +
                '\n' +
            ...
            new Article(2, 1,'When the dynamic repricing is impossible for my Product?', 'There are several possible cases when dynamic price updating is impossible:\n' +
                '\n' +
            ...
            new Article(3, 1,'Sale Price settings and the Repricing Service', 'The Sale Price settings are not compatible with the dynamic Price updating.\n' +
                '\n' +
        ...
        new Knowledge(2, 'Other', [
            new Article(1, 2,'M2E Pro Cron executing with LiteSpeed web-server', 'In the latest versions of Litespeed server default settings for External Application Abort were changed. In  result Litespeed server closes connection and synchronization is not completed successfully.\n' +
                '\n' +
            ...
            new Article(2, 2,'Issues with M2E Pro Server Connection', 'some cases, when you try to run any action (for example, Synchronization tasks, List/Relist/Revise/Stop actions) the error about M2E Pro Server connection fail can arise. You can get one of the following messages:\n' +
                '\n' +
        ...
        new Knowledge(3, 'Amazon Integration', [
            new Article(1, 3,'Magento Order Amount differs from Amazon Order Amount   ', 'The situation when Magento Order Amount differs from Amazon Order Amount happens due to the next settings:\n' +
                '\n' +
            ...
            new Article(3, 3,'I can\'t choose certain attribute because it is not presented in the list', 'For the fields which are required for Amazon, the only common attributes, those which are presented in all attribute sets are shown in the dropdown.\n' +
                '\n' +
            ...
            new Article(4, 3,'Product Upload Speed to Amazon', 'The amazon API works not the same way as eBay API does. The process is slow because of Amazon workflow - when request is sent to Amazon, their servers do not process them immediately as eBay do, but put in a queue. And it may take from 5 minutes to several hours until Amazon processes request and sends response. \n' +
            ...
            new Article(5, 3,'Amazon: Price and QTY shown as "N/A" in the grid', 'The "Not Listed" is displayed temporary.\n' +
                '\n' +
            ...
            new Article(6, 3,'Disable notification for Amazon orders', 'Q: Is it possible to disable notification for Amazon orders as it is against Amazon Policy.\n' +
                '\n' +
            ...
            new Article(7, 3,'Inactive (Blocked) status for Amazon Item', 'Item is marked as Inactive (Blocked) in the following situations:\n' +
                '\n' +
            ...
            new Article(11, 3,'Condition is invalid or missed error for Amazon Item', 'Please check if you have set Condition Settings in Listing http://docs.m2epro.com/x/1YcVAQ for M2E Pro M1 and http://docs.m2epro.com/x/XwItAQ for M2E Pro M2')
        ]),
        new Knowledge(4, 'eBay Integration', [
            new Article(1, 4,'Can\'t upload product image on eBay', 'This problem does not concern to m2e pro.\n' +
                '\n' +
            ...
            new Article(2, 4,'Create multiple eBay items from the same magento product [for Magento v 1.x]', 'It is possible to list the same product multiple times and the inventory will be synchronized for all listed products.\n' +
                '\n' +
            ...
            new Article(4, 4,'Adding VAT to price', 'The VAT settings can be set in Configuration -> Policies -> Price, Quantity and Format policy > VAT Rate.\n' +
                '\n' +
        ...
        new Knowledge(5, 'Configurations', [
            new Article(1, 5,'Data Communication via M2E Pro Server', 'M2E Pro service is provided based on a client-server network architecture. Which means the data communication is performed following a certain scenario M2E Pro server acts as intermediary between the Magento and the channel servers.\n' +
                '\n' +
            ...
            new Article(2, 5,'Another Synchronization Is Already Running', 'The warning “Another Synchronization Is Already Running” appears in the Synchronization Log, if it is time to start new synchronization, but the previous is not finished yet. So system waits until the current synchronization process will finish.\n' +
                '\n' +
                'Normally, you should ignore this warning.'),
            new Article(3, 5,'Error: Order was not created. Reason: Please specify a shipping method.', 'You should check if shipping method M2E Pro eBay Shipping  enabled  in System > Configuration > Shipping Methods or Stores > Configuration > Sales > Shipping Methods for the store view where order is being imported.\n' +
                '\n' +
    ...
    private ideas: Idea[] = [
        new Idea(1, 'Amazon Repricing Service', [
            new Suggestion(1, 1,(Math.round(Math.random() * 20)), 'Seller Fulfilled Prime', 'add a custom scenario that will compete with SFP', 'Aaron Englander ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(2, 1,(Math.round(Math.random() * 20)), 'Rule to beat buy box', 'It would be great to have a rule which would automatically reduce the product price in increments until either the buy box is achieved or the minimum price is reached.', 'Stuart ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
        ]),
        new Idea(2, 'Other Sales Channel', [
            new Suggestion(1, 2,(Math.round(Math.random() * 20)), 'Integration with Etsy', 'Any plans in the future?', 'Janice Bailey ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(2, 2,(Math.round(Math.random() * 20)), 'idealo.de Direktkauf', 'Germanys biggest price comparison allows customer with the feature "idealo Direktkauf" the checkout on their plattform', 'TheNittyGritty ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
            ...
            new Suggestion(3, 2,(Math.round(Math.random() * 20)), 'Integration with eBid', 'eBid now have a pretty comprehensive API and they\'re starting to rank pretty well in Google SERPS.\n' +
                '\n' +
            ...
            new Suggestion(4, 2,(Math.round(Math.random() * 20)), 'Integration with Real.de (used to be Hitmeister)', 'big german retail store chain bought and expanded the hitmeister online marketplace\n' +
                '\n' +
                'https://www.real.de/\n' +
                '\n' +
                'Rest API\n' +
            ...
            new Suggestion(5, 2,(Math.round(Math.random() * 20)), 'Integration with Houzz.com Marketplace', 'Houzz market place is growing so fast in home furnishing industry. The amount of Houzz orders sellers receive for home decor items is much more than any other marketplace even amazon. Thank you.', 'KT ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(6, 2,(Math.round(Math.random() * 20)), 'Integration with Sears', 'Sears supposedly has similar sales potential as buy.com', 'Chris Haupt ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(7, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(8, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(9, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(10, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(11, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(12, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
            new Suggestion(13, 2,(Math.round(Math.random() * 20)), 'Integration with Newegg', 'Please, create an Integration with the Newegg', 'Dmitry ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
        ]),
        new Idea(3, 'eBay Integration', [
            new Suggestion(1, 3,(Math.round(Math.random() * 20)), 'eBay Plus Germany', 'eBay Plus is similar to Amazon Prime. There is no possibility for many products to activate this with M2E. This possibility should exist in the future.', 'TiWe ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null),
            ...
            new Suggestion(2, 3,(Math.round(Math.random() * 20)), 'Import product views value from Ebay', 'Ebay does not allow the merchant to sort listings based on views. It would be great if M2E could bring this data into Magento where it can be sorted and analyzed. \n' +
                '\n' +
        ...
        new Idea(4, 'Amazon Integration', [
            ...
            new Suggestion(3, 4,(Math.round(Math.random() * 20)), 'Amazon Buy Box Notification', 'It would be great if we could see in the Magento CMS that shows if your are the chosen vendor for the buy box on Amazon for a particular listing.', 'Jamie ', new Date().getTime(), [
                new Comments(1, 'John Doe', new Date().getTime(), 'Same comment...', null)
            ]),
        ])
    ];
